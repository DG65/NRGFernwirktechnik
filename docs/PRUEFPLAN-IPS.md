...
| 7 | Bei Last (viele Sollwerte in rascher Folge, parallel Generalabfragen) | keine Semaphore-Timeouts, alle beantwortet | ✅ 22.09.2026, `LOAD=200` (zwei Verbindungen, eine aktiv): 200 Sollwerte innerhalb 1 s gesendet, alle 200 innerhalb weniger Sekunden korrekt beantwortet (keine negativ), dazwischen 17 Generalabfragen ebenfalls beantwortet, beide Verbindungen blieben stabil, 7/7 Prüfungen. Deckt nur die belegten 7 Punkte ab (nicht 100+ echte Datenpunkte gleichzeitig) — echte Last mit vielen belegten Punkten weiterhin offen. **Fund dabei:** ein abgebrochener Testlauf hinterlässt eine TCP-Verbindung ohne STARTDT, die sich erst über t0 (30 s) auflöst – normales, aber merkbares Verhalten |
